...
if( $contact == 1 ){
    ...
    add_filter('manage_sunset-contact_posts_columns','set_sunset_contact_columns');
    add_action('manage_sunset-contact_posts_custom_column','set_sunset_custom_column',10,2);
...
function set_sunset_contact_columns($columns)
{
    $newcolumns=array();
    $newcolumns['title']= 'First Name';
...
function set_sunset_custom_column($column, $post_id)
{
    switch($column){
        case('message'):
